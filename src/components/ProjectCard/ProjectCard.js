...
import { Info, GitHub } from '@material-ui/icons';
...
export default function ProjectCard({ project }) {
	...
	return (
		<Grid item xs={13} sm={7} md={5} lg={4} xl={3}>
			<Card>
				<CardContent>
					...
					<div className='chip-container'>
						{project.tags.map((label, i) => (
							<Chip className='chip' size='small' label={label} color='primary' key={i} />
						))}
					</div>
				</CardContent>
				<CardActions disableSpacing className='card-actions'>
					<IconButton title='More Info' aria-label='More Info' onClick={handleOpen}>
						<Info />
					</IconButton>
					{modalOpen ? (
